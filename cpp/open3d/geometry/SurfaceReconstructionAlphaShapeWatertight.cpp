...
std::shared_ptr<TriangleMesh> TriangleMesh::CreateFromPointCloudAlphaShapeWatertight(
    ...
    utility::LogDebug(
            ...
            "satisfy constraint");
    struct IndexWithRadius{
        size_t idx; double radius;
    };
    std::vector<IndexWithRadius> tidxes;
    ...
    for (size_t tidx = 0; tidx < tetra_mesh->tetras_.size(); ++tidx){
        ...
        double r = std::sqrt(dx * dx + dy * dy + dz * dz - 4 * a * c) /
                (2 * std::abs(a));
        tidxes.push_back(IndexWithRadius{tidx,r});
    }
    std::sort(tidxes.begin(),tidxes.end(),[](const IndexWithRadius &a, const IndexWithRadius &b) { return a.radius < b.radius;});

    for (const IndexWithRadius& ir : tidxes) {
        const auto& tetra = tetra_mesh->tetras_[ir.idx];
        if (ir.radius > alpha) break;
